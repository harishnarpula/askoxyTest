...
export async function fetchProjects(): Promise<CodeFile[]> {
  const response = await fetch(`http://localhost:9876/api/vibecode-service/projects`);
  if (!response.ok) throw new Error("Failed to fetch projects");
  return response.json();
}

export async function fetchProjectTitles(): Promise<string[]> {
  const response = await fetch(`http://localhost:9876/api/vibecode-service/projects/titles`);
...
export async function fetchProjectChildren(projectId: string): Promise<CodeFile[]> {
  const response = await fetch(`http://localhost:9876/api/vibecode-service/projects/${projectId}/children`);
...
export async function fetchFolderFiles(folderId: string): Promise<CodeFile[]> {
  const response = await fetch(`http://localhost:9876/api/vibecode-service/folder/${folderId}/files`);
...
export async function fetchProjectFiles(title: string): Promise<CodeFile[]> {
  const response = await fetch(`http://localhost:9876/api/vibecode-service/projects?title=${encodeURIComponent(title)}`);
...
export async function fetchFileContent(fileId: string): Promise<string> {
  const response = await fetch(`http://localhost:9876/api/vibecode-service/file/${fileId}/content`);
